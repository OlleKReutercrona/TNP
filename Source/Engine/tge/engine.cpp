...
, myWantToUpdateSize(false)
{
	...
	// if aWindowConfiguration has values different from the default values of EngineConfiguration, they are used, otherwise Settings::GetEngineConfiguration() is used
	// a result of that is that it isn't possible to force EngineConfiguration values to be the default values
	// a fix: if aWindowConfiguration is provided, set all values based on it. To override only some values, use GetEngineConfiguration on the outside.
	...
	{ // if specified for incomming parameters, prioritize
		...
		}
		{
			...
			//cp.myActivateDebugSystems = (cp.myActivateDebugSystems == param.myActivateDebugSystems) ? config.myActivateDebugSystems : param.myActivateDebugSystems;
		}
